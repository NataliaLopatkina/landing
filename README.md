...
### _pages
Аналогично _blocks.
**Исключение:** Если в проекте используется общий файл стилей, то он должен находится в: `_pages/common/common.css`.
...
После запуска сервера переходим по `localhost:8080`. Приложение автоматически перезагрузится, если измените какой-либо из исходных файлов.
